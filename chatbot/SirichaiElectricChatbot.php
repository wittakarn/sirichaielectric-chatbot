...
class SirichaiElectricChatbot extends GeminiChatbot {
    ...
    protected function getFunctionDeclarations(): array {
        return array(array('functionDeclarations' => array(
            array(
                'name'        => 'search_catalog',
                'description' => 'Look up catalog category names relevant to the customer\'s product question. Used in WORKFLOW 1 Path B — when the customer\'s message has NO specific model/part number or series name, only a product type, brand, or spec. If the message DOES contain a model/part number (e.g. "LRD05", "WEG5001K") OR a series/product line name (e.g. "TeSys D", "W22", "Easy9", "IE3"), skip this function and call search_products directly with loose terms (Path A). Returns the closest matching catalog category lines — use them verbatim to call search_products; whether the results count as a LOOSE MATCH (requiring the "ไม่พบ" prefix) is determined by the PRESENTING RESULTS rule in the system prompt, not by this function\'s output alone. Pass the customer\'s product question as-is — no keyword extraction or rewriting needed. Full path/selection rules live in the system prompt (WORKFLOW 1 Path A & Path B).',
                'parameters'  => array(
                    'type'       => 'object',
                    'properties' => array(
                        'query' => array(
                            'type'        => 'string',
                            'description' => 'The customer\'s product question, passed through as-is. Use only when the question has NO model/part number and NO series name. Example: "ขอราคาเบรกเกอร์ 32A" or "สายไฟ THW สีดำ ยาซากิ" or "เบรกเกอร์ ABB 3P". (Queries containing a model code like "LRD05" or a series name like "TeSys D" should go to search_products directly — do NOT call search_catalog for those.)'
                        )
                    ),
            ...
            array(
                ...
                'description' => 'Search for products. The criterias array accepts either: (a) loose terms — a model/part number or series name and optionally a brand name (e.g. ["WEG5001K","PANASONIC"] or ["TeSys D","Schneider"]) — used in WORKFLOW 1 Path A when the customer\'s message contains a specific model/part code or series/product line name; or (b) exact catalog category names returned from search_catalog() — used in WORKFLOW 1 Path B. Returns results as lines formatted: "Name | Price | Unit | Id". Use the numeric Id (4th field) to render every product as a markdown link: "[Name](https://shop.sirichaielectric.com/product/Id) ราคา: Price บาท/Unit". Maximum 3 criteria items.',
                'parameters'  => array(
                    'type'       => 'object',
                    'properties' => array(
                        'criterias' => array(
                            ...
                            'description' => 'Array of search criteria — either loose terms (model/part number or series name, brand) for Path A, or EXACT category names returned from search_catalog() for Path B. Maximum 3 items. Never mix terms from different products in one call.'
                        )
                    ),
